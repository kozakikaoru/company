...
| Subagent | 責務 | 発動タイミング |
|---|---|---|
| `plan` | 仕様検討 / 設計 / 技術選定 / UX 判断 / 収益モデル | 「どう設計?」「技術は?」「これでいい?」 |
| `implement` | コード実装・修正・リファクタ (GO 後のみ) | 「作って」「直して」「動くようにして」 |
| `verify` | テスト実行/作成、レビュー、型/lint、セキュリティ、a11y | `implement` の直後に自動呼び出し + 「レビューして」 |
| `research` | Web 検索、競合/市場、ライブラリ、エラー原因、コードベース探索 | 「調べて」「〇〇どこにある?」「このエラーは?」 |
| `explain` | 既存コード/仕様/概念の解説、比較、判断理由の再構築 | 「これって何?」「なぜこう?」「教えて」 |
...
## commit mode / local mode の切替

デフォルトは **ローカル運用** (`.company/` は `.gitignore` で除外)。

**コミット運用に切替** — 例: スマホから続き作業したい

```
「スマホから開発したい」「.company もコミットして」「クラウドでも同期して」
```

と言えば秘書ちゃんが `.gitignore` から除外解除 + `git add .company/` + commit を実行。以後 `git pull` すればどこからでも続き作業可能。

⚠️ 注意: コミット運用にすると仕様書・議事録が git 履歴 (=GitHub) に乗る。プライベートリポジトリ推奨。

**ローカルだけに戻す**

```
「ローカルだけに戻して」「gitignore に戻して」
```
...
- 9 サブエージェント → 5 サブエージェントに削減、旧 agent 名は使えない (`pm` → `plan`、`engineer` → `implement`、`qa` + `security` → `verify` に統合、等)
