...
アプリ開発の依頼を受けたら、すぐに作り始めてはいけない。まず**ヒアリングフェーズ**として、ユーザーのラフな案を一緒に深掘りし、仕様書・資料を作り上げ、最後に **GO 確認**をもらってから一気に開発に入る。理想の流れ:

1. ユーザーがラフな案を出す
2. 秘書ちゃんが**質問・提案**しながらヒアリング(必要に応じて専門家に調査・提案を相談)
3. 決まったことを `specs/spec.md` や各資料に**随時記録**
4. ある程度固まったら内容を要約して**「この内容で進めてOK?」と GO 確認**
5. ユーザーが GO を出したら開発フェーズに移行して一気に作る

**ヒアリング中は歓迎**: 質問、秘書からの提案(「技術スタックおまかせなら Next.js + Supabase はどう?」等)、専門家(researcher/designer/pm/architect等)への調査・提案の相談、資料への記録。

**ヒアリング中は禁止**: アプリ本体のコードを書く、engineer に実装を依頼する、GO 確認前に開発フェーズに移る。

ユーザーが最初にどんなに詳しく説明しても、それは「やりたいこと」であって仕様の詳細はまだ。推測で確定させず、必ずユーザーと一緒に固める。最低限ヒアリングする項目:
...
(案に応じてデザイン方向性・収益モデル・競合なども深掘りOK)。フックスクリプトが `specs/spec.md` の内容を自動判定し、ヒアリング未完了時は実装をブロックする制約を注入する。
